...
def fileAnalysis(filePath, biasPath, darkPath, thres, thresp, savePath, x=2, y=2, autoThres=False):
    ...
    for c in range(0, len(files)):
        ...
        nx, ny = phot.shape  # find size of image
        f1 = np.reshape(phot, nx * ny)  # change the shape to be 1d
        q = np.argsort(f1)[::-1] # Returns indices of f1 where the intensity is highest, sorted in decreasing order
        # print(f1[q[0]])
        # print(q[0])
        # go through pixels with large signal
        j = 0
        ...
        while above_thres:
            ...
            if f1[i] >= t:  # pixel is above threshold
                ...
                yR = (i % 1936) # Find the (x,y) coordinates of this event
                ...
                if (xR > dx) and (xR < nx - dx - 1) and (yR > dy) and (yR < ny - dy - 1):
                    # p = sum(f1[i+di])
                    p = 0
                    for xi in range(xR - dx, xR + dx + 1):  # Look in 5x5 Region
...
def func_gaussq(x, norm, cent, width, quad1, quad2, quad3):
    return norm * np.exp(-(x - cent) ** 2 / (2 * width ** 2)) + (quad1 * x ** 2 + quad2 * x + quad3)


def func_gausse(x, norm, cent, width, const):
    return (norm * np.exp(-(x - cent) ** 2 / (2 * width ** 2))) + (np.exp(const * x))


gausse = lambda p, x: p[0] * np.exp(-(x - p[1]) ** 2 / (2 * p[2] ** 2)) + (np.exp(p[3] * x))



def fitgause(ph, cent, fwhm, plot=0):
    """
     Fit a Gaussian + Exponeneital (np.exp(constant*x))
     :param ph: Event data
     :param cent: Initial guess at centroid
     :param fwhm: Initial guess at FWHM
     :param plot: If plot >1 it will plot outpout
     :return: [Fitting results],[Fitting Errors]
     """
    ...
    p, t = curve_fit(func_gausse, chan, counts, p0=pinit, sigma=cerr)
    error = []
    for i in range(len(p)):
        try:
            error.append(np.absolute(t[i][i]) ** 0.5)
        except:
          error.append( 0.00 )
    perr_curvefit = np.array(error)
    norm, cent, width, cons = p
    cont_curve = np.linspace(min(chan), max(chan), 1e8)
    fit_curve = func_gausse(cont_curve, norm, cent, width, cons)
    peak_counts = max(fit_curve)
    half = .5 * peak_counts
    idx_fwhm = np.argwhere(abs(fit_curve - half) < .00005)
    idx_cent = np.where(fit_curve == max(fit_curve))
    u_cent = cont_curve[idx_cent]
    print(idx_fwhm)
    #try:
     #   FWHM = abs(cont_curve[idx_fwhm[0][0]] - cont_curve[idx_fwhm[-1][0]])
    #except IndexError:
    FWHM = 2.35*width
    print('Gaussian fit = ', p)
    if (plot > 0):
        fig = plt.figure(plot, figsize=(18, 6))
        ax = fig.add_subplot(1, 2, 1)
        ax.errorbar(chan, counts, cerr, fmt='.', capsize=0)
        ax.set_xlabel('Pulse height (ADU)')
        ax.set_ylabel('Number events/bin')
        textstr = '\n'.join((
            r'Counts= %.2f +- %.2f' % (aduToeV(norm,m,b),aduToeV(perr_curvefit[0],m,b)),
            r'Centroid = %.2f +- %.2f' % (aduToeV(u_cent,m,b),aduToeV(perr_curvefit[1],m,b)),
            r'FWHM = %.2f +- %.2f' %(aduToeV(FWHM,m,b),aduToeV(perr_curvefit[2]*2.35,m,b)), r'Const = %.2f +- %.2f' % (aduToeV(cons,m,b),aduToeV(perr_curvefit[3],m,b))))
        # these are matplotlib.patch.Patch properties
        props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)

        # place a text box in upper left in axes coords
        ax.text(.15, .89, textstr, fontsize=8, ha='center', va='center', transform=ax.transAxes, bbox=props)
        ax.plot(chan, gausse(p, chan))
    return [norm,u_cent,FWHM,cons],[perr_curvefit[0],perr_curvefit[1],perr_curvefit[2],perr_curvefit[3]]


gaussc = lambda p, x: p[0] * np.exp(-(x - p[1]) ** 2 / (2 * p[2] ** 2)) + p[3]


gaussq = lambda p, x: p[0] * np.exp(-(x - p[1]) ** 2 / (2 * p[2] ** 2)) + (p[3] * x ** 2 + p[4] * x + p[5])


def fitgausq(ph, cent, fwhm, plot=0):
    """
    Fit a Gaussian + Quadratic (At^2 + Bt + c)
    :param ph: Event data
    :param cent: Initial guess at centroid
    :param fwhm: Initial guess at FWHM
    :param plot: If plot >1 it will plot outpout
    ...
    sig = fwhm / 2.35
    # fit a gaussian+constant to the data, use Gehrels variance function for error
    pinit = [tot / (sig * np.sqrt(2 * np.pi)), cent, sig, 0, 0, 0]  # use calculated statistics as first guess for fit
    cerr = 1 + np.sqrt(counts + 0.75)
    p, t = curve_fit(func_gaussq, chan, counts, p0=pinit, sigma=cerr, maxfev=5000)
    error = []
    for i in range(len(p)):
        try:
            error.append(np.absolute(t[i][i])**0.5)
            ...
            error.append(0.00)
    perr_curvefit = np.array(error)
    norm, cent, width, quad1, quad2, quad3 = p
    #cont_curve = np.linspace(min(chan), max(chan), 1e8)
    #fit_curve = func_gaussq(cont_curve, norm, cent, width, quad1, quad2, quad3)
    #half = .5 * peak_counts
    #idx_fwhm = np.argwhere(abs(fit_curve - half) < .0005)
    #FWHM = abs(cont_curve[idx_fwhm[0][0]] - cont_curve[idx_fwhm[-1][0]])
    FWHM = 2.35*width
    #idx_cent = np.where(fit_curve == max(fit_curve))
    #u_cent = cont_curve[idx_cent[0][0]]
    print(FWHM)
    print('Gaussian fit = ', p)
    if (plot > 0):
        fig = plt.figure(plot, figsize=(18, 6))
        ...
        ax.set_ylabel('Number events/bin')
        textstr = '\n'.join((
            r'Counts= %.2f +- %.2f  ' % (norm,perr_curvefit[0]),
            r'Centroid = %.2f +- %.2f eV' % (aduToeV(cent,m,b),aduToeV(perr_curvefit[1],m,b)),
            r'FWHM = %.2f +- %.2f eV' % (aduToeV(FWHM,m,b),aduToeV(perr_curvefit[2],m,b)), r'Const1 = %.2f +- %.2f' % (aduToeV(quad1,m,b),aduToeV(perr_curvefit[3],m,b)),
            r'Const2 = %.2f +- %.2f' % (aduToeV(quad2,m,b),aduToeV(perr_curvefit[4],m,b)), r'Const3 = %.2f +- %.2f' % (aduToeV(quad3,m,b),aduToeV(perr_curvefit[5],m,b))))
        # these are matplotlib.patch.Patch properties
        props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)

        # place a text box in upper left in axes coords
        ax.text(.19, .87, textstr, fontsize=8, ha='center', va='center', transform=ax.transAxes, bbox=props)

        # plt.title(filename)
        # s1 = 'Mean = %.1f' % average(v)
        # s2 = 'FWHM = %.1f' % 2.35(np.std(v))
        # print(s2)
        # s4 = 'Counts = %i' % len(v)
        # plt.text((max(chan)-min(chan))*0.75+min(chan), max(counts)*0.8, s1+'\n'+s2+'\n'+s3+'\n'+s4)
        ax.plot(chan, gaussq(p, chan))
    return [norm, cent, FWHM, quad1,quad2,quad3], [perr_curvefit[0], perr_curvefit[1], perr_curvefit[2], perr_curvefit[3],perr_curvefit[4],perr_curvefit[5]]


gaussc = lambda p, x: p[0] * np.exp(-(x - p[1]) ** 2 / (2 * p[2] ** 2)) + p[3]


def fitgausc(ph, cent, fwhm, plot=0):
    """
     Fit a Gaussian + Constant
     :param ph: Event data
     :param cent: Initial guess at centroid
     :param fwhm: Initial guess at FWHM
     :param plot: If plot >1 it will plot outpout
    ...
    norm, cent, width, cons = p
    cont_curve = np.linspace(min(chan), max(chan), 1e8)
    fit_curve = func_gaussc(cont_curve, norm, cent, width, cons)
    peak_counts = max(fit_curve)
    half = .5 * peak_counts
    idx_fwhm = np.argwhere(abs(fit_curve - half) < .00005)
    idx_cent = np.where(fit_curve == max(fit_curve))
...
def Load_all(Temp,path):
    """Create CSV files for ep,ex,ey for all images in directory
    Returns a list of ep,ex,ey for all images (each index is a temp value in the list)
    Parameters:
        Temp: List of integers with temperature ranges in directory
        m: Slope of Intensity vs eV
        b: Y int of Intensity vs eV
        path: full file pathway into directory
     """

    epl = []
    exl = []
    eyl = []
    pathb = '/Volumes/My Passport/CMOS Research/CMOSsensor/CMOSsensor/cImages/10_23_Doxygen/Bias'
    pathd = '/Volumes/My Passport/CMOS Research/CMOSsensor/CMOSsensor/cImages/10_23_Doxygen/Dark'
    for j in range(len(Temp)):
        if Temp[j] > 0:
            pathv = path + '/' + '%s' % (Temp[j]) + 'Deg' + 'Test'
        if Temp[j] < 0:
            pathv = path + '/' + 'Neg' + '%s' % abs((Temp[j])) + 'Deg' + 'Test'
        if Temp[j] == 0:
            pathv = path + '/' + '%s' % (Temp[j]) + 'Deg' + 'Test'
        print(Temp)
        print(pathv)
        print(pathd)
        print(pathb)
        ep, ex, ey = fileAnalysis(pathv.replace('-', ''), pathb.replace('-', ''), pathd.replace('-', ''), 49, 15,
                                  '10_23_%sDegGlass' % (Temp[j]))
        epl.append(ep)
        exl.append(ex)
        eyl.append(ey)
    return (epl, exl, eyl)
